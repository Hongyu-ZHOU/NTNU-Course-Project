...
typedef RT_MODEL_heli_q8_2_1_2_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * heli_q8_2_1_2(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(heli_q8_2_1_2_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(heli_q8_2_1_2_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://heli_q8_2_1_2:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: heli_q8_2_1_2 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        fflush(stdout);
        result = hil_task_start(heli_q8_2_1_2_DW.HILReadEncoderTimebase_Task,
          (t_clock) heli_q8_2_1_2_P.HILReadEncoderTimebase_clock, 500.0, -1)
          ;
        if (result == 0) {

...
typedef RT_MODEL_heli_q8_4_3_output_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * heli_q8_4_3_output(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(heli_q8_4_3_output_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(heli_q8_4_3_output_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: heli_q8_4_3_output -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        result = hil_task_start
          (heli_q8_4_3_output_DW.HILReadEncoderTimebase_Task, (t_clock)
           heli_q8_4_3_output_P.HILReadEncoderTimebase_clock, 500.0, -1)
          ;
        if (result == 0) {

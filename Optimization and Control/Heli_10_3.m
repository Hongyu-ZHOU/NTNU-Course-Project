...
q = 1;
R_c = q;
G = blkdiag(kron(eye(N),Q),kron(eye(N),R_c));
...
% ylabel('u');
% subplot(512);
% plot(time,travel_opt,'m',time,travel_opt,'mo');
% ylabel('lambda');
% subplot(513);
% plot(time,travel_rate_opt,'m',time,travel_rate_opt,'mo');
% ylabel('r');
% subplot(514);
% plot(time,pitch_opt,'m',time,pitch_opt,'mo');
% ylabel('p');
% subplot(515);
% plot(time,pitch_rate_opt,'m',time,pitch_rate_opt,'mo');
% xlabel('time(s)');
% ylabel('p_c');
% grid on;
...
state_opt = [travel_opt; travel_rate_opt; pitch_opt; pitch_rate_opt];
heli_state = [time state_opt'];


% closed loop
figure(2);
plot(Output_10_3.time,Output_10_3.signals.values(:,1));
hold on;
plot(time',travel_opt,'-o');

figure(3);
plot(Output_10_3.time,Output_10_3.signals.values(:,3));
hold on;
...
figure(1)
plot(time,travel_opt,'-o',Output_10_3.time,Output_10_3.signals.values(:,1));
axis([0 30 -0.5 3.5]);
...
figure(2)
plot(time,pitch_opt,'-o',Output_10_3.time,Output_10_3.signals.values(:,3));
axis([0 30 -0.6 0.8]);
legend('p*','p');
xlabel('time(s)');

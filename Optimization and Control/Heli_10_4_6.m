...
u_l = [-30*pi/180 -inf]';
u_u = [30*pi/180 inf]';
x_l = [-inf -0.6 -25*pi/180 -inf -inf -0.075]';
x_u = [inf 0.6 25*pi/180 inf inf 0.075]';
...
figure(4)
plot(lambda_k,c_con,'--o');
hold on;
plot(lambda_k,e_k);
hold on;
plot(Output.signals.values(:,1),Output.signals.values(:,5),'k');
legend('c','e*','e')
axis([-0.5 pi 0 0.2]);
xlabel('Travel angle(rad)')
ylabel('Elevation angle(rad)');
% plot(Output.signals.values(2500:10626,5));
